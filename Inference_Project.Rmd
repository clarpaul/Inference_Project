...
Simulation is consistent with theory.  
  ...
  1.  The statistics of the distribution of sample means are approximately equal to their theoretical values, consistent with the properties of means and variances  
...
In **Figure 4**, our sample data is plotted on the vertical axis, and the associated normal quantiles (standard deviations from center) are plotted on the horizontal axis. The associated normal quantiles are calculated as follows: Empirical Cumulative Densities ($ECD_{q}$ values) of the sample data are calculated, and those Densities are used to calculate a normal quantile by `qnorm(ECD)`. Specifically, to obtain the cumulative densities, sample data points $i$ are ascendingly ordered from $1 \:to\: n$, then each point $i$ is related to a cumulative probability associated with the mid-point between points $i$ and $i-1$, calculated as $ECD_{q}=(i-0.5)/n$, where $n$ is the number of sampled values, and $q$ is the standard normal quantile calculated from each $ECD$. This way, for example, point $i = 500$, which has sample quantile (data value) $\approx 5$, is associated with cumulative probability `0.4995 = (500-0.5)/1000` = $ECD_{q} \approx$ `pnorm(q=0) = 0`, and so on. That relationship is inverted (e.g., via `qnorm(ECD)`) to make connections with the *Standard Normal Quantiles* $q$ on the horizontal axis of the plot.
...
## Preliminaries
We first load the primary package needed in this analysis and the specific dataset we will use. Note that for this analysis we also make use of ggplot2, loaded for part I of this project.
```{r}
library(datasets)
data("ToothGrowth")
```
